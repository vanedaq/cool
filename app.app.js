...
class Finanzas {
  constructor(){
    ...
    this.iniYM="2025-08";
    // mes seleccionado: si hay guardado, úsalo; si no, iniYM
    ...
    this.bindUI();
    this.buildMonths();
    // crear meses intermedios hasta el mes seleccionado y render
    this.createMissingMonthsUpTo(this.mes);
    this.renderAll();
  ...
  bindUI(){
    ...
    if(this.sel) this.sel.addEventListener("change",(e)=>{ this.mes=e.target.value; localStorage.setItem(this.selKey,this.mes); this.createMissingMonthsUpTo(this.mes); this.renderAll(); this.toast("Mes cambiado"); });
  ...
  /* Crea meses intermedios desde el último mes existente hasta target (inclusive) */
  createMissingMonthsUpTo(target){
    try{
      // si ya existe target, asegúrate de que prev months exist too
      // find the latest month present
      const months = Object.keys(this.data).sort();
      if(months.length===0){
        this.ensureMonth(target);
        return;
      }
      // compute start month: max(latest existing, iniYM)
      const latest = months[months.length-1];
      // if latest >= target, nothing to do; else create months between latest+1 ... target
      const toNum = m => { const [y,mo]=m.split('-').map(Number); return y*12+mo; };
      let curNum = toNum(latest);
      const targetNum = toNum(target);
      for(let n = curNum+1; n<=targetNum; n++){
        const y = Math.floor((n-1)/12);
        // Above formula not right; do incremental date approach instead:
      }
      // Simpler: iterate using date objects from (latest + 1 month) until target
      let [ly,lm] = latest.split('-').map(Number);
      let d = new Date(ly, lm-1, 1);
      const [ty,tm] = target.split('-').map(Number);
      const targetD = new Date(ty, tm-1, 1);
      d.setMonth(d.getMonth()+1); // move to next month after latest
      while(d <= targetD){
        const k = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
        this.ensureMonth(k);
        d.setMonth(d.getMonth()+1);
      }
      // finally ensure target itself
      this.ensureMonth(target);
    }catch(e){
      console.error('createMissingMonthsUpTo error', e);
    }
  }

  /* ensureMonth: si el mes no existe crea copiando desde el mes previo.
     Reglas: copia fijos, ingresos, compras, ahorros siempre; copia tarjetas/creditos solo si cuotasPagadas < numeroCuotas.
     Reasigna id y actualiza fecha al primer dia del mes nuevo; recalcula cuota mensualmente.
  */
  ensureMonth(key){
    ...
    if(this.data[prev]){
      ...
      const newMonthPrefix = `${key}-01`;

      const copyArray = (arr, kind) => {
        ...
        return arr.map(item=>{
          ...
          it.id = this.uid();
          // update date
          if(it.fecha) it.fecha = newMonthPrefix;
          // reset paid flag for new month (optional) — keep paid false so user marks again
          it.paid = false;
          // for debts: if finished (cuotasPagadas >= numeroCuotas) don't copy
          if(kind === 'tarjetas' || kind === 'creditos'){
            const cuotas = parseInt(it.numeroCuotas||0) || 0;
            const pagadas = parseInt(it.cuotasPagadas||0) || 0;
            if(cuotas>0 && pagadas >= cuotas) return null; // filter out fully paid
            // recalc cuota
            const monto = Number(it.montoTotal || it.monto || 0);
            const tasa = Number(it.tasaMensual || 0) || 0;
            const aval = Number(it.avalPct || 0) || 0;
            const ivaAval = Number(it.ivaAvalPct || 0) || 0;
            it.cuotaMensual = this.cuota(monto, tasa, cuotas, aval, ivaAval);
          }
          return it;
        }).filter(Boolean);
      };
      ...
      this.data[key] = {
        ingresos: copyArray(copy.ingresos,'ingresos'),
        gastosFijos: copyArray(copy.gastosFijos,'gastosFijos'),
        tarjetas: copyArray(copy.tarjetas,'tarjetas'),
        creditos: copyArray(copy.creditos,'creditos'),
        gastosCompras: copyArray(copy.gastosCompras,'gastosCompras'),
        ahorros: copyArray(copy.ahorros,'ahorros')
      };
    } else {
  ...
  togglePaid(key,id){
    const list=this.mesData[key]; const it=list.find(x=>x.id===id); if(!it) return;
    it.paid = !it.paid;
    // Si es deuda y marcamos pagado, opcionalmente incrementamos cuotasPagadas
